...
import type {CookingSession, Ingredient, Recipe, RecipeRecommendationRequest, User} from '../types';
...
export const ingredientApi = {
  ...
  scanImageBase64: async(imageBase64: string): Promise<Ingredient[]> => {
    ...
    const response =
        await api.post('/ingredients/scan-base64', {image: base64Data});
    return response.data;
  },
  ...
  addIngredient: async(
      ingredient: Partial<Ingredient>, userId: string): Promise<Ingredient> => {
    try {
  ...
  addIngredientsBatch: async(
      ingredients: Partial<Ingredient>[], userId: string):
      Promise<Ingredient[]> => {
        ...
        try {
          ...
          console.warn(
              'Batch add failed, falling back to parallel single adds:', e);
          const settled = await Promise.allSettled(ingredients.map(
  ...
  updateIngredient: async(
      ingredientId: string, ingredient: Partial<Ingredient>,
      userId: string): Promise<Ingredient> => {
...
export const recipeApi = {
  getRecommendations: async(request: RecipeRecommendationRequest):
      Promise<Recipe[]> => {
  ...
  createRecipe: async(recipe: Partial<Recipe>, userId?: string):
      Promise<Recipe> => {
  ...
  addToShoppingList: async(
      ...
      Promise<{message: string; id: string}> => {
        const response =
            await api.post('/shopping-list', data, {params: {user_id: userId}});
        return response.data;
      },
  ...
  deleteShoppingListItem: async(userId: string, itemId: string):
      Promise<{message: string}> => {
        const response = await api.delete(
            `/shopping-list/${itemId}`, {params: {user_id: userId}});
        return response.data;
      },

  toggleShoppingListItem: async(
      userId: string, itemId: string, itemIndex: number): Promise<any> => {
    const response = await api.post(
        `/shopping-list/${itemId}/toggle-item`, null,
        {params: {user_id: userId, item_index: itemIndex}});
    return response.data;
  },

  completeShoppingList: async(
      userId: string, itemId: string): Promise<{message: string}> => {
    const response = await api.put(
        `/shopping-list/${itemId}/complete`, null, {params: {user_id: userId}});
    return response.data;
  },

  getRecipePages: async(userId: string, ingredients?: string[]):
  ...
  saveRecipePage: async(userId: string, pageData: {
    page_index: number; recipes: Recipe[]; ingredients: string[];
  }): Promise<{message: string; page_index: number}> => {
    const response =
        await api.post('/recipe-pages', pageData, {params: {user_id: userId}});
    return response.data;
  },

  clearRecipePages: async(userId: string): Promise<{message: string}> => {
    const response =
        await api.delete('/recipe-pages', {params: {user_id: userId}});
    return response.data;
  },

  clearShoppingList: async(userId: string): Promise<{message: string}> => {
    const response =
        await api.delete('/shopping-list', {params: {user_id: userId}});
    return response.data;
  },
};

export const cookingApi = {
  startSession: async(recipeId: string, userId: string):
  ...
  advanceStep: async(
      ...
      manualAdvance?: boolean): Promise<CookingSession> => {
  ...
  pauseSession: async(sessionId: string, userId: string):
      Promise<CookingSession> => {
        const response =
            await api.post(`/cooking/session/${sessionId}/pause`, null, {
              params: {user_id: userId},
            });
        return response.data;
      },

  resumeSession: async(sessionId: string, userId: string):
      Promise<CookingSession> => {
        const response =
            await api.post(`/cooking/session/${sessionId}/resume`, null, {
              params: {user_id: userId},
            });
        return response.data;
      },
  ...
  completeSession: async(sessionId: string, userId: string):
      Promise<CookingSession> => {
        const response =
            await api.post(`/cooking/session/${sessionId}/complete`, null, {
              params: {user_id: userId},
            });
        return response.data;
      },
